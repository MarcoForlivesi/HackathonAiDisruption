...
namespace UnrealHttpServer
{
	...
	FHttpRouteHandle FWebUtil::BindRoute(const TSharedPtr<IHttpRouter>& HttpRouter, FString Path, const EHttpServerRequestVerbs& Verb, const FHttpResponser& HttpResponser)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("Binding router: %s\t%s"), *VerbString, *Path);

		// check if HTTP path is valid
		FHttpPath HttpPath(Path);
		if (!HttpPath.IsValidPath())
		{	
			UE_LOG(LogTemp, Warning, TEXT("Invalid http path: %s"), *Path);
#if WITH_EDITOR
			if (GEngine != nullptr)
			{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Bind failed: %s\t%s"), *VerbString, *Path);
			return nullptr;
		}
#if WITH_EDITOR
		if (GEngine != nullptr)
	...
	TSharedPtr<FJsonObject> FWebUtil::GetRequestJsonBody(const FHttpServerRequest& Request)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("caught request not in utf-8 application/json body content!"));
			return nullptr;
		}
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("failed to parse request string to json: %s"), *RequestBodyString);
			return nullptr;
		}
		return RequestBody;
	}
